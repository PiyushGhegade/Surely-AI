...
export const RegisterBox = () => {
  ...
  useEffect(() => {
    // upon refresh (after the login), if the user is logged in, we need to trigger onSignup and redirect
    if (user) {
      const redirectReason = searchParams.get("redirect-reason");
      const targetUrl =
        redirectReason === "create-checker" ? "/dashboard" : "/checkers";
      handleErr(trpcClient.user.onSignup.mutate(), () => {
        router.push(targetUrl);
      });
    }
  }, [user, searchParams, trpcClient, router]);
